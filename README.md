# スクールチャイム管理システム (School Chime Scheduler)
...
*   **データの永続化と共有**:
    ...
    *   **GitHub連携機能**:
        ...
        *   **アカウントがない場合**: GitHubアカウントは無料で作成できます。作成しない場合は、Google Drive等のURLを使用することで設定の永続化が可能です。
...
## 🔰 付録: GitHubアカウント作成手順 (初心者向け)

GitHubの画面は英語ですが、以下の手順通りに進めれば迷わず無料で作成できます。

1.  **公式サイトへアクセス**:
    *   GitHub.com にアクセスし、右上の **「Sign up」** (登録) ボタンをクリックします。

2.  **基本情報の入力**:
    *   画面に `Enter your email` と表示されたら、**メールアドレス**を入力して `Continue` を押します。
    *   `Create a password` と表示されたら、**パスワード**を決めて入力し `Continue` を押します。
        *   ※条件: 15文字以上、または「8文字以上かつ数字と小文字を含む」必要があります。
    *   `Enter a username` と表示されたら、**ユーザー名**（username）を英数字で入力して `Continue` を押します。
        *   ※すでに使われている場合はエラーが出るので、別の名前に変えてください。
        *   ※これがアプリ内の**GitHub連携設定**画面の「**Owner (ユーザー名)**」になります。
    *   `Email preferences` (お知らせメール) は、不要なら `n` を入力して `Continue` を押します。

3.  **ロボット認証 (パズル)**:
    *   `Verify your account` の下の **「Start puzzle」** をクリックします。
    *   「矢印を使って動物を回転させ、手の向きに合わせてください」などのパズルが出題されます。指示通りに操作して `Submit` を押します。

4.  **アカウント作成とメール認証**:
    *   パズルが完了したら、一番下の **「Create account」** ボタンをクリックします。
    *   登録したメールアドレスにGitHubから数字のコードが届くので、その数字を画面に入力すれば完了です！

5.  **リポジトリの作成（データの保存場所）**:
    *   ログイン後の画面右上にある **「+」** アイコンをクリックし、**「New repository」** を選びます。
    *   **Repository name** に好きな名前（例: `school-chime-data`）を入力します。
        *   ※これがアプリ内の「**Repository (リポジトリ名)**」になります。
    *   **Public** (公開) か **Private** (非公開) を選びます。
        *   ※自分だけが使うなら **Private** がおすすめです。
    *   一番下の **「Create repository」** をクリックすれば準備完了です！
    *   ※この後、アプリの設定画面で「ユーザー名」と「リポジトリ名」を入力する際に、ここで決めた名前を使います。